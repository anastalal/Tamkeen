...
<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-8s gap-5 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" wire:navigate>
                        {{ __('لوحة التحكم') }}
                    </x-nav-link>
                    <x-nav-link :href="route('quiz')"  :active="request()->routeIs('quiz')" wire:navigate>
                        {{ __('محلل الاهتمامات') }}
                    </x-nav-link>
                    <x-nav-link  :active="request()->routeIs('tool')" wire:navigate>
                        {{ __('ادوات القياس') }}
                    </x-nav-link>
                    <x-nav-link  :active="request()->routeIs('tool')" wire:navigate>
                        {{ __('حجز الاستشارات') }}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link  :href="route('quiz')"  :active="request()->routeIs('quiz')" wire:navigate>
                {{ __('محلل الاهتمامات') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link  :active="request()->routeIs('tool')" wire:navigate>
                {{ __('ادوات القياس') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link  :active="request()->routeIs('tool')" wire:navigate>
                {{ __('حجز الاستشارات') }}
